...
from utils.ml_utils import train_and_save_model
...
def launch_attack():  # Implement this
    ...
    pct = 1
    rowcount = int(total_rows * pct)

    train_data.drop(train_data.tail(total_rows - rowcount).index, inplace = True)

    train_data['default payment next month'] = [1 - train_data.at[i, 'default payment next month'] for i in range(rowcount)]
