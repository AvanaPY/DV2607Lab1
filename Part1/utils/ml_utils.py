...
def store_data(submitted_data):
    ...
    #-------------------- Implementation av försvar ---------------------------
    train_data_dropped_labels = train_data.drop(columns=["default payment next month"])
    #--------------------------------------------------------------------------
    ...
    for entry in submitted_data[1:]:
        try:
            ...
            # # Implementation av försvar
            if not safe_to_add(validated_entry, train_data_dropped_labels):
                failed_entries += 1
            else:
...
def safe_to_add(data_point, train_data_dropped_labels):
    """
        Inlämningsuppgift, del 1.5.2, implementation av försvar

        Denna funktion tar in en datapunkt samt all träningsdata fast med en borttagen label kolonn. 
        Sedan tar den bort label från datapunkten och jämför med hela datasettet om den datan redan finns där.

        Returnerar True/False
    """
    # Drop label
    del data_point["default payment next month"]

    # Turn into numpy array
    as_np = np.array(list(data_point.values())).astype("int")
    
    # Compare
    data_exists = (train_data_dropped_labels == as_np).all(1).any()
